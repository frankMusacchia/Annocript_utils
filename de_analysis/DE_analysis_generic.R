...
#R CMD BATCH --no-save --no-restore '--args <input_folder>  <counts_filtered> <organism_name> <target_file> <minFDR> <analysis_type> <annocript_out>' DE_analysis_generic.R
...
length(args)
if (length(args) < 5){
	stop("USAGE: R CMD BATCH --no-save --no-restore '--args <input_folder> <counts_filtered> <organism_name> <target_file> <minFDR> <analysis_type> <annocript_out>' DE_analysis_generic.R")
}
...
analType = 	as.numeric(args[6])

#It the Annocript output is used
ann_out_used = 6
if (length(args) > 6){
	ann = args[7]#"chaetoceros_transcriptome_cpm1per2_uniref_2014_08_filt_ann_out.txt"
}
...
get_diff_exp = function (couple){
  ...
  exp_name = paste(paste(couple,collapse="_"),fdrString,"significant.txt",sep="_")
  exp_name
  # Espressione differenziale tra le due fasi
  # Execute the statistical test (t-test) to see differences between different samples
...
get_expression_table = function (couple){
  ...
  exp_name = paste(paste(couple,collapse="_"),"expression",sep="_")
  exp_name
...
join_with_annotation = function (exp_name){
	...
	table_and_ann_out = merge(tableA,ann_out,by="row.names",all.x=T, sort=F)
